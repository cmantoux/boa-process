...
H_range     = [0, 1, 2, 5]
s_range     = [0.2, 0.8]
eps_range   = [0., 0.01, 0.02, 0.05] # /!\ CAUTION : IN E8-E9, eps REPRESENTS A FALSE POSITIVE (reps. NEGATIVE) RATE
# In E9, we add false negatives
...
for e, eps in enumerate(eps_range):
    sim_index[5] = e
    sim_name = os.environ["B"]+f"/E{EXP}/E{EXP}_"+"".join(map(str, sim_index))+"_noisy.pkl"
    ...
    def generate_data(N, T, p_ext, H, s, eps, seed):
        ...
        observations, seed_ages, extinctions = src.simulation.simulate(p_ext, H, N, T, init, 0)
        # Add false positive
        FN = 1*(np.random.rand(*observations.shape) < eps)
        observations = np.minimum(observations, 1-FN)
        return observations, seed_ages, extinctions
