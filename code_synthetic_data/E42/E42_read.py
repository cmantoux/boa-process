...
EXP = 42 # experiment number (used for the seed)
...
N_range     = [30, 50, 100]
T_range     = [5, 10]
p_ext_range = np.linspace(-0.1, 0.1, 11)
H_range     = [0, 1, 2, 5]
s_range     = [0.2, 0.8]
            ...
            for d in range(len(H_range)):
                for e in range(len(s_range)):
                    for f in range(len(eps_range)):
                        for g in range(K):
                            sim_indices[d].append([a, b, c, d, e, f, g])
...
for i, H in enumerate(H_range):
    ...
    for sim_index in tqdm(sim_indices[i]):
        ...
        p_ext = p_c[H] + p_ext_range[sim_index[2]] # p_ext is centered around the true value of H
        ...
        result_array.data[a, b, c, 0, e, f, g, results.index("rmse_p_ext")] = rmse_p_ext
        result_array.data[a, b, c, 0, e, f, g, results.index("p_ext_mean")] = hist["p_ext"].mean()
        result_array.data[a, b, c, 0, e, f, g, results.index("p_ext_std")] = hist["p_ext"].std()
        result_array.data[a, b, c, 0, e, f, g, results.index("H_recovery")] = H_recovery
        result_array.data[a, b, c, 0, e, f, g, results.index("H_inf")] = H_inf
        result_array.data[a, b, c, 0, e, f, g, results.index("extinction_test_1")] = test_1
        result_array.data[a, b, c, 0, e, f, g, results.index("extinction_test_2")] = test_2
        result_array.data[a, b, c, 0, e, f, g, results.index("eps_mean")] = hist["eps"].mean()
        result_array.data[a, b, c, 0, e, f, g, results.index("eps_std")] = hist["eps"].std()
        result_array.data[a, b, c, 0, e, f, g, results.index("rmse_eps")] = rmse_eps
        for h in range(H_max+1):
            result_array.data[a, b, c, 0, e, f, g, results.index(f"p(H={h})")] = H_probs[h]
            result_array.data[a, b, c, 0, e, f, g, results.index(f"s_{h}_mean")] = hist["s_mean"][h]
            result_array.data[a, b, c, 0, e, f, g, results.index(f"s_{h}_rmse")] = hist["s_rmse"][h]

        del hist

    f = open(os.environ["B"]+f"/E42_results_{H}.pkl", "wb")
